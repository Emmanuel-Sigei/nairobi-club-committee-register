...
export async function sendMeetingNotification(recipients: Array<{ email: string; name: string }>, input: { type: "scheduled" | "updated" | "cancelled"; title: string; committeeName: string; startAt: Date; endAt: Date; timezone: string; location: string; meetingId: string }): Promise<void> {
  ...
  for (const recipient of recipients) {
    await sendZohoMail({ toAddress: recipient.email, subject: `Nairobi Club: ${subjectPrefix} Ã¢â‚¬â€ ${input.title}`, content: `<p>Dear ${escapeHtml(recipient.name)},</p><p><strong>${escapeHtml(subjectPrefix)}</strong></p><p><strong>Committee:</strong> ${escapeHtml(input.committeeName)}</p><p><strong>Meeting:</strong> ${escapeHtml(input.title)}</p><p><strong>When:</strong> ${escapeHtml(start)} Ã¢â‚¬â€œ ${escapeHtml(end)} (${escapeHtml(input.timezone)})</p><p><strong>Location:</strong> ${escapeHtml(input.location)}</p><p><a href="${appMeetingUrl}">Open meeting in the Committee Register</a></p><p>Regards,<br>Nairobi Club ICT</p>` });
  }
}
...
export async function sendCoiCorrectionNotification(
  ...
  await sendZohoMail({
    toAddress: email,
    subject: `Nairobi Club: conflict-of-interest record corrected Ã¢â‚¬â€ ${meeting.title}`,
...
export async function sendAttendanceCorrectionNotification(
  ...
  await sendZohoMail({
    toAddress: email,
    subject: `Nairobi Club: attendance record corrected - ${meeting.title}`,
    content: `<p>Dear ${escapeHtml(name)},</p>
<p>Your attendance record has been corrected by an administrator.</p>
<p><strong>Committee:</strong> ${escapeHtml(meeting.committeeName)}<br>
<strong>Meeting:</strong> ${escapeHtml(meeting.title)}<br>
<strong>When:</strong> ${escapeHtml(when)}</p>
<p><strong>Previous status:</strong> ${escapeHtml(input.previousStatus)}<br>
<strong>Corrected status:</strong> ${escapeHtml(input.correctedStatus)}</p>
<p><strong>Correction reason:</strong> ${escapeHtml(input.correctionReason)}</p>
<p>The correction is retained in the audit history.</p>
...
export async function sendDocumentAddedNotification(
  ...
  const url = input.meetingId
    ? `${appUrl()}/meetings/${encodeURIComponent(input.meetingId)}`
    : appUrl();

  for (const recipient of recipients) {
    await sendZohoMail({
      toAddress: recipient.email,
      subject: `Nairobi Club: new document - ${input.title}`,
      content: `<p>Dear ${escapeHtml(recipient.name)},</p>
<p>A new committee document has been added to the Nairobi Club Committee Register.</p>
<p><strong>Committee:</strong> ${escapeHtml(input.committeeName)}<br>
${input.meetingTitle ? `<strong>Meeting:</strong> ${escapeHtml(input.meetingTitle)}<br>` : ""}
<strong>Document:</strong> ${escapeHtml(input.title)}<br>
<strong>Version:</strong> ${input.version}<br>
<strong>File:</strong> ${escapeHtml(input.fileName)}</p>
<p><a href="${url}">Open the Committee Register</a></p>
<p>Regards,<br>Nairobi Club ICT</p>`,
    });
  }
}