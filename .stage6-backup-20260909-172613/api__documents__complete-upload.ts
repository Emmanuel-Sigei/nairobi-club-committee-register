...
export default async function handler(
  ...
  try {
    const body = await readJson<CompleteUploadBody>(request);
    ...
    const objectKey = requiredDocumentString(
      body.objectKey,
      "objectKey",
    );
    ...
    const expectedPrefix =
      `${committee.slug}/${meetingId ?? "general"}/`;

    if (!objectKey.startsWith(expectedPrefix)) {
      return error("Invalid document object key.", 400);
    }

    let head;

    try {
      head = await headR2Object(objectKey);
    } catch {
      return error(
    ...
    if (
      head.ContentType &&
      head.ContentType.toLowerCase() !== contentType
    ) {
      return error(
    ...
    const scopeKey = documentScopeKey(meetingId);
