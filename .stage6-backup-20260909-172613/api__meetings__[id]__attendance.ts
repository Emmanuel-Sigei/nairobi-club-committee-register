...
async function syncZohoRsvps(request: Request, context: Awaited<ReturnType<typeof getAuthenticatedUser>>, meeting: NonNullable<Awaited<ReturnType<typeof getMeeting>>>, force: boolean) { if (!context) throw new Error("Authentication required."); if (context.user.role !== "ADMIN") throw new Error("Administrator access required."); if (meeting.status !== "SCHEDULED") throw new Error("RSVP synchronization is only available while the meeting is scheduled."); const stale = !meeting.lastSyncedAt || Date.now() - meeting.lastSyncedAt.getTime() >= SYNC_STALE_MS; if (!force && !stale) return { synced: false, createdDrafts: 0, lastSyncedAt: meeting.lastSyncedAt }; const rsvps = await readZohoRsvps(meeting); const eligible = await getEligibleMembers(meeting.id, meeting.committeeId, meeting.startAt); const existing = await getDb().meetingAttendance.findMany({ where: { meetingId: meeting.id } }); const existingByUser = new Map(existing.map((record) => [record.userId, record])); let createdDrafts = 0; for (const member of eligible) { const response = rsvps.get(member.email.toLowerCase()); const current = existingByUser.get(member.id); if (response === "DECLINED" && !current) { await getDb().meetingAttendance.create({ data: { meetingId: meeting.id, userId: member.id, status: "APOLOGY_DRAFT", source: "ZOHO_SYNC", reason: "Draft created from Zoho Calendar RSVP decline. Awaiting Admin confirmation." } }); createdDrafts += 1; } } const syncedAt = new Date(); await getDb().meeting.update({ where: { id: meeting.id, status: "SCHEDULED" }, data: { lastSyncedAt: syncedAt, lastSyncedById: context.user.id } }); await writeAuditEvent({ request, context, action: "ZOHO_RSVP_SYNCED", entityType: "Meeting", entityId: meeting.id, metadata: { createdDrafts, attendeeCount: rsvps.size, forced: force, syncedAt } }); return { synced: true, createdDrafts, lastSyncedAt: syncedAt }; }
async function getAttendanceResponse(request: Request, meetingId: string, allowAutoSync = true) { const context = await getAuthenticatedUser(request); if (!context) return error("Authentication required.", 401); const meeting = await getMeeting(meetingId); if (!meeting) return error("Meeting not found.", 404); if (!(await canViewCommittee(context, meeting.committeeId))) return error("You do not have access to this committee.", 403); let syncResult: { synced: boolean; createdDrafts: number; lastSyncedAt: Date | null } = { synced: false, createdDrafts: 0, lastSyncedAt: meeting.lastSyncedAt }; if (allowAutoSync && context.user.role === "ADMIN" && meeting.status === "SCHEDULED") { try { syncResult = await syncZohoRsvps(request, context, meeting, false); } catch (caught) { if (!(caught instanceof CalendarNotConfiguredError)) console.error("On-demand Zoho RSVP sync failed.", caught); } } const refreshed = await getMeeting(meetingId); if (!refreshed) return error("Meeting not found.", 404); const eligible = await getEligibleMembers(refreshed.id, refreshed.committeeId, refreshed.startAt); const records = await getDb().meetingAttendance.findMany({ where: { meetingId }, include: { markedBy: { select: { id: true, name: true } } } }); const byUser = new Map(records.map((record) => [record.userId, record])); let rsvpByEmail = new Map<string, string>(); if (context.user.role === "ADMIN" && refreshed.status === "SCHEDULED") { try { rsvpByEmail = await readZohoRsvps(refreshed); } catch { rsvpByEmail = new Map(); } } return json({ success: true, meeting: { id: refreshed.id, status: refreshed.status, startAt: refreshed.startAt, endAt: refreshed.endAt, committeeId: refreshed.committeeId, lastSyncedAt: refreshed.lastSyncedAt, closedAt: refreshed.closedAt }, attendance: eligible.map((member) => ({ user: member, attendance: byUser.get(member.id) ?? null, zohoRsvp: rsvpByEmail.get(member.email.toLowerCase()) ?? null })), sync: syncResult }); }
...
async function handleWrite(request: Request, meetingId: string) {
  ...
  if (action === "reject-draft") { if (context.user.role !== "ADMIN") return error("Administrator access required.", 403); const draft = await getDb().meetingAttendance.findUnique({ where: { meetingId_userId: { meetingId, userId: requestedUserId } } }); if (!draft || draft.status !== "APOLOGY_DRAFT") return error("No pending apology draft exists for this member.", 404); await getDb().meetingAttendance.update({ where: { id: draft.id }, data: { status: "APOLOGY_DRAFT_REJECTED", source: "ADMIN", reason: reason ?? draft.reason, markedAt: new Date(), markedById: context.user.id } }); await writeAuditEvent({ request, context, action: "APOLOGY_DRAFT_REJECTED", entityType: "MeetingAttendance", entityId: draft.id, metadata: { meetingId, userId: requestedUserId, reason: reason ?? null } }); return getAttendanceResponse(request, meetingId, false); }
  if (action === "mark") { if (context.user.role !== "ADMIN") return error("Administrator access required.", 403); if (!(await ensureEligible(meeting, requestedUserId))) return error("The selected user was not a member of this committee for this meeting.", 400); const requestedStatus = parseStatus(body.status); const existing = await getDb().meetingAttendance.findUnique({ where: { meetingId_userId: { meetingId, userId: requestedUserId } } }); if (existing?.status === "APOLOGY_DRAFT" && requestedStatus === "EXCUSED") { await getDb().meetingAttendance.update({ where: { id: existing.id }, data: { status: "APOLOGY", source: "ADMIN", markedAt: new Date(), markedById: context.user.id } }); await writeAuditEvent({ request, context, action: "APOLOGY_DRAFT_CONFIRMED", entityType: "MeetingAttendance", entityId: existing.id, metadata: { meetingId, userId: requestedUserId, previousStatus: existing.status, confirmationPath: "manual-mark" } }); return getAttendanceResponse(request, meetingId, false); } if (existing?.status === "APOLOGY" && requestedStatus !== "EXCUSED") return error("An in-app apology is authoritative and cannot be overwritten by a manual attendance mark.", 409); const record = await upsertAttendance(meetingId, requestedUserId, requestedStatus, "ADMIN", context.user.id, reason); await writeAuditEvent({ request, context, action: existing ? "ATTENDANCE_UPDATED" : "ATTENDANCE_MARKED", entityType: "MeetingAttendance", entityId: record.id, metadata: { meetingId, userId: requestedUserId, previousStatus: existing?.status ?? null, previousSource: existing?.source ?? null, status: requestedStatus, source: "ADMIN", reason: reason ?? null } }); return getAttendanceResponse(request, meetingId, false); }
  return error("Unsupported attendance action.", 400);
}
export default async function handler(request: Request, context: { params: { id?: string } | Promise<{ id?: string }> }): Promise<Response> { try { const params = await context.params; const meetingId = params.id; if (!meetingId) return error("Meeting ID is required.", 400); if (request.method === "GET") return getAttendanceResponse(request, meetingId); if (request.method === "POST") return await handleWrite(request, meetingId); return error("Method not allowed.", 405); } catch (caught) { console.error("Attendance request failed.", caught); if (caught instanceof CalendarNotConfiguredError) return error(caught.message, 503, "CALENDAR_NOT_CONFIGURED"); if (caught instanceof Error) return error(caught.message, 400); return error("Unable to process attendance request.", 500); } }
