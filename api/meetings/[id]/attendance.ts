...
function fieldName(_value: unknown): string {
  return "value";
}
...
async function getEligibleMembers(meetingId: string, committeeId: string, startAt: Date) {
  ...
  return Array.from(unique.values()).map((membership) => ({
    ...membership.user,
...
function attendeeEmail(attendee: unknown): string | null {
  if (typeof attendee !== "object" || attendee === null) return null;
  const value = (attendee as Record<string, unknown>).email;
  return typeof value === "string" ? value.trim().toLowerCase() : null;
}

function attendeeStatus(attendee: unknown): string | null {
  if (typeof attendee !== "object" || attendee === null) return null;
  const value = (attendee as Record<string, unknown>).status;
  return typeof value === "string" ? value.toUpperCase() : null;
}

async function readZohoRsvps(meeting: NonNullable<Awaited<ReturnType<typeof getMeeting>>>) {
  if (!meeting.committee.zohoCalendarId || !meeting.zohoEventUid) {
    throw new CalendarNotConfiguredError();
  }

  const snapshot = await getCalendarEventSnapshot(
    meeting.committee.zohoCalendarId,
    meeting.zohoEventUid,
  );

  const attendees = Array.isArray(snapshot.resource.attendees)
    ? snapshot.resource.attendees
    : [];

  const byEmail = new Map<string, string>();
  for (const attendee of attendees) {
    const email = attendeeEmail(attendee);
    const status = attendeeStatus(attendee);
    if (email && status) byEmail.set(email, status);
  }

  return byEmail;
}

async function syncZohoRsvps(
  request: Request,
  context: Awaited<ReturnType<typeof getAuthenticatedUser>>,
  ...
  if (meeting.status !== "SCHEDULED") {
    throw new Error("RSVP synchronization is only available while the meeting is scheduled.");
  }

  const stale = !meeting.lastSyncedAt || Date.now() - meeting.lastSyncedAt.getTime() >= SYNC_STALE_MS;
  if (!force && !stale) {
    return { synced: false, createdDrafts: 0, lastSyncedAt: meeting.lastSyncedAt };
  }
  ...
  for (const member of eligible) {
    const response = rsvps.get(member.email.trim().toLowerCase());
    const current = existingByUser.get(member.id);

    if (response === "DECLINED" && !current) {
      await getDb().meetingAttendance.create({
        data: {
          meetingId: meeting.id,
          userId: member.id,
          status: "APOLOGY_DRAFT",
          source: "ZOHO_SYNC",
          reason: "Draft created from Zoho Calendar RSVP decline. Awaiting Admin confirmation.",
        },
      });
      createdDrafts += 1;
    }
  ...
  await getDb().meeting.update({
    ...
    data: {
      ...
      lastSyncedById: context.user.id,
    },
  });
  ...
  await writeAuditEvent({
    request,
    context,
    action: "ZOHO_RSVP_SYNCED",
    entityType: "Meeting",
    entityId: meeting.id,
    metadata: {
      createdDrafts,
      attendeeCount: rsvps.size,
      forced: force,
      syncedAt,
    },
  });
...
async function getAttendanceResponse(
  request: Request,
  meetingId: string,
  allowAutoSync = true,
) {
  ...
  let syncResult: { synced: boolean; createdDrafts: number; lastSyncedAt: Date | null } = {
    synced: false,
    createdDrafts: 0,
    lastSyncedAt: meeting.lastSyncedAt,
  };

  if (allowAutoSync && context.user.role === "ADMIN" && meeting.status === "SCHEDULED") {
    try {
      syncResult = await syncZohoRsvps(request, context, meeting, false);
    } catch (caught) {
      if (!(caught instanceof CalendarNotConfiguredError)) {
        console.error("On-demand Zoho RSVP sync failed.", caught);
      }
    }
  }
  ...
  const records = await getDb().meetingAttendance.findMany({
    where: { meetingId },
    include: {
  ...
  if (context.user.role === "ADMIN" && refreshed.status === "SCHEDULED") {
    try {
      rsvpByEmail = await readZohoRsvps(refreshed);
    } catch {
      rsvpByEmail = new Map();
    }
  }

  const rows = eligible.map((member) => ({
    user: member,
    attendance: byUser.get(member.id) ?? null,
    zohoRsvp: rsvpByEmail.get(member.email.toLowerCase()) ?? null,
  }));
  ...
  return json({
    ...
    meeting: {
      ...
      committeeId: refreshed.committeeId,
...
async function upsertAttendance(
  meetingId: string,
  userId: string,
  status: AttendanceWriteStatus | "APOLOGY",
  source: "SELF" | "ADMIN",
  markedById: string,
  reason?: string,
) {
  return getDb().meetingAttendance.upsert({
...
async function handleWrite(request: Request, meetingId: string) {
  ...
  if (action === "sync") {
    ...
    try {
      const result = await syncZohoRsvps(request, context, meeting, true);
      return getAttendanceResponse(request, meetingId, false).then(async (response) => {
        if (!response.ok) return response;
        const payload = await response.json();
        return json({ ...payload, sync: result });
      });
    } catch (caught) {
      if (caught instanceof CalendarNotConfiguredError) {
        return error(caught.message, 503, "CALENDAR_NOT_CONFIGURED");
      }
      if (caught instanceof Error) return error(caught.message, 409);
      return error("Unable to synchronize Zoho RSVP responses.", 500);
    }
  }

  if (meeting.status === "CANCELLED") {
    return error("Cancelled meetings cannot receive attendance changes.", 409);
  }
  ...
  if (action === "close") {
    ...
    if (!meeting.lastSyncedAt || Date.now() - meeting.lastSyncedAt.getTime() >= SYNC_STALE_MS) {
      try {
        ...
        if (caught instanceof Error) return error(`Meeting cannot be closed until Zoho RSVP synchronization succeeds: ${caught.message}`, 503);
    ...
    await getDb().$transaction(async (tx) => {
      for (const member of eligible) {
        ...
        } else if (current.status === "APOLOGY_DRAFT") {
    ...
    await writeAuditEvent({
      ...
      entityType: "Meeting",
      entityId: meetingId,
      metadata: { eligibleMembers: eligible.length },
    });
  ...
  if (meeting.status === "CLOSED") {
    if (action !== "mark" || context.user.role !== "ADMIN") {
      return error("Attendance records are locked because this meeting is closed.", 409);
    }
    ...
    const current = await getDb().meetingAttendance.findUnique({
      where: { meetingId_userId: { meetingId, userId: requestedUserId } },
    });
    if (!current) return error("No attendance record exists to correct.", 404);

    await getDb().meetingAttendance.update({
      where: { id: current.id },
      data: {
        status,
        source: "ADMIN",
  ...
  if (action === "check-in") {
    ...
    await writeAuditEvent({
      ...
      entityType: "MeetingAttendance",
      metadata: { meetingId, userId: context.user.id, status: "PRESENT", source: "SELF" },
    });
    return getAttendanceResponse(request, meetingId, false);
  }
  ...
  if (action === "apology") {
    ...
    if (existing && existing.source === "SELF" && existing.status !== "APOLOGY_DRAFT") {
      return error("An attendance record already exists for this meeting.", 409);
    }
    await upsertAttendance(meetingId, context.user.id, "APOLOGY", "SELF", context.user.id, reason);
    await writeAuditEvent({
      request,
      context,
      action: "APOLOGY_SUBMITTED",
      entityType: "MeetingAttendance",
      metadata: { meetingId, userId: context.user.id, status: "APOLOGY", source: "SELF" },
    });
    return getAttendanceResponse(request, meetingId, false);
  }
  ...
  if (action === "reject-draft") {
    ...
    await writeAuditEvent({
      request,
      context,
      action: "APOLOGY_DRAFT_REJECTED",
      entityType: "MeetingAttendance",
      entityId: draft.id,
      metadata: { meetingId, userId: requestedUserId, reason: reason ?? null },
    });
    return getAttendanceResponse(request, meetingId, false);
  }
  ...
  if (action === "mark") {
    ...
    if (existing?.status === "APOLOGY" && status !== "APOLOGY") {
      return error("An in-app apology is authoritative and cannot be overwritten by a manual attendance mark.", 409);
    }
    await upsertAttendance(meetingId, requestedUserId, status, "ADMIN", context.user.id, reason);
    await writeAuditEvent({
      request,
      context,
...
export default async function handler(
  ...
  try {
    ...
    if (request.method === "GET") {
      return getAttendanceResponse(request, meetingId);
    }
    if (request.method === "POST") {
      return await handleWrite(request, meetingId);
    }
