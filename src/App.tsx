...
import { CLUB_LOGO_URL } from "./branding";
...
function Button({ children, onClick, type = "button", variant = "secondary", disabled = false }: { children: ReactNode; onClick?: () => void; type?: "button" | "submit"; variant?: "primary" | "secondary" | "danger" | "ghost"; disabled?: boolean }) { const classes = { primary: "bg-slate-900 text-white hover:bg-slate-800 border-slate-900", secondary: "bg-white text-slate-800 hover:bg-slate-50 border-slate-300", danger: "bg-red-700 text-white hover:bg-red-800 border-red-700", ghost: "bg-transparent text-slate-600 hover:bg-slate-100 border-transparent" }; return <button type={type} onClick={onClick} disabled={disabled} className={`inline-flex min-h-10 items-center justify-center rounded-lg border px-4 py-2 text-sm font-semibold transition disabled:cursor-not-allowed disabled:opacity-50 ${classes[variant]}`}>{children}</button>; }
...
function Header({
  ...
  return (
    <header className="border-b border-[#C8A45D]/60 bg-[#0B1F3A] shadow-sm">
      <div className="mx-auto max-w-7xl px-6">
        <div className="flex min-h-24 items-center justify-between gap-6">
          ...
          >
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#D8BC7A]">
                Nairobi Club
              </p>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden text-right sm:block">
              ...
              <p className="text-xs text-[#D8BC7A]">
                {roleLabel(user.role)}
              </p>
            </div>
        ...
        <div className="flex gap-1 overflow-x-auto border-t border-white/10 py-2 md:hidden">
          {navigation.map((item) => (
            <button
              key={item.id}
              type="button"
...
function DetailCard({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-slate-200 p-4"><p className="text-[11px] font-semibold uppercase tracking-[0.10em] text-slate-500">{label}</p><p className="mt-1 text-sm font-medium leading-6 text-slate-900">{value}</p></div>; }
function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) { return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><h2 className="text-xl font-semibold text-slate-950">{title}</h2><button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-900">x</button></div>{children}</div></div>; }
function Overview({ user, meetings, committees, onMeetings, onCreateMeeting }: { user: { name: string; role: UserRole }; meetings: Meeting[]; committees: Committee[]; onMeetings: () => void; onCreateMeeting: () => void }) { const [now] = useState(() => Date.now()); const scheduled = meetings.filter((m) => m.status === "SCHEDULED"); const upcoming = scheduled.filter((m) => +new Date(m.startAt) >= now).sort((a, b) => +new Date(a.startAt) - +new Date(b.startAt)); return <div className="space-y-8"><section className="rounded-xl border border-slate-200 bg-white p-7 shadow-sm"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Your committees</p><h2 className="mt-2 text-[26px] font-semibold tracking-tight">Welcome, {user.name.split(" ")[0]}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">Plan meetings, record attendance and keep committee records in one place.</p></div>{user.role === "ADMIN" && <Button variant="primary" onClick={onCreateMeeting}>Schedule meeting</Button>}</div></section><section className="grid gap-4 md:grid-cols-3"><StatCard label="Committees" value={String(committees.length)} detail="Committees in your view" /><StatCard label="Scheduled" value={String(scheduled.length)} detail="Meetings scheduled" /><StatCard label="Upcoming" value={String(upcoming.length)} detail="Coming up next" /></section><section className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 px-6 py-5"><div><h3 className="font-semibold">Next meetings</h3><p className="mt-1 text-sm text-slate-500">Meetings coming up next.</p></div><Button variant="ghost" onClick={onMeetings}>View all</Button></div>{upcoming.length === 0 ? <EmptyState title="No upcoming meetings" description="Nothing is scheduled yet. New meetings will appear here." /> : <div className="divide-y divide-slate-100">{upcoming.slice(0, 5).map((m) => <div key={m.id} className="flex flex-col gap-3 px-6 py-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">{m.committee.name}</p><h4 className="mt-1 font-semibold">{m.title}</h4><p className="mt-1 text-sm text-slate-600">{formatDateTime(m.startAt, m.timezone)} - {m.location}</p></div><Badge tone="success">Scheduled</Badge></div>)}</div>}</section></div>; }
function StatCard({ label, value, detail }: { label: string; value: string; detail: string }) { return <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.10em] text-slate-500">{label}</p><p className="mt-3 text-[26px] font-semibold">{value}</p><p className="mt-1 text-sm text-slate-500">{detail}</p></div>; }
function MeetingsView({ meetings, committees, user, onRefresh, onCreate }: { meetings: Meeting[]; committees: Committee[]; user: { id: string; role: UserRole }; onRefresh: () => Promise<void>; onCreate: () => void }) { const initialPathMeetingId = window.location.pathname.match(/^\/meetings\/([^/]+)$/)?.[1] ? decodeURIComponent(window.location.pathname.match(/^\/meetings\/([^/]+)$/)?.[1] ?? "") : null; const [filter, setFilter] = useState<MeetingFilter>("upcoming"); const [committeeId, setCommitteeId] = useState(""); const [selectedId, setSelectedId] = useState<string | null>(initialPathMeetingId); const [now] = useState(() => Date.now()); const filtered = useMemo(() => [...meetings].filter((m) => { if (committeeId && m.committeeId !== committeeId) return false; if (filter === "cancelled") return m.status === "CANCELLED"; if (filter === "closed") return m.status === "CLOSED"; if (filter === "upcoming") return m.status === "SCHEDULED" && +new Date(m.startAt) >= now; if (filter === "past") return m.status === "SCHEDULED" && +new Date(m.endAt) < now; return true; }).sort((a, b) => +new Date(a.startAt) - +new Date(b.startAt)), [meetings, filter, committeeId, now]); const selected = selectedId ? meetings.find((m) => m.id === selectedId) ?? null : null; if (selected) return <MeetingDetail meeting={selected} user={user} onBack={() => { setSelectedId(null); if (window.location.pathname !== "/") window.history.replaceState({}, "", "/"); }} onRefresh={onRefresh} />; return <div className="space-y-6"><section className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Meetings</p><h2 className="mt-2 text-[26px] font-semibold">Committee meetings</h2><p className="mt-2 text-sm text-slate-600">Plan meetings, review attendance and keep RSVP responses together.</p></div>{user.role === "ADMIN" && <Button variant="primary" onClick={onCreate}>Schedule meeting</Button>}</section><section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-col gap-3 lg:flex-row"><div className="flex flex-wrap gap-2">{([['upcoming','Upcoming'],['past','Past'],['closed','Closed'],['cancelled','Cancelled'],['all','All']] as Array<[MeetingFilter,string]>).map(([value,label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`rounded-lg px-3 py-2 text-sm font-semibold ${filter === value ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600"}`}>{label}</button>)}</div><div className="lg:ml-auto lg:w-72"><Select value={committeeId} onChange={(e) => setCommitteeId(e.target.value)}><option value="">All committees</option>{committees.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</Select></div></div></section><section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">{filtered.length === 0 ? <EmptyState title="No meetings found" description="No meetings match these filters." /> : <div className="divide-y divide-slate-100">{filtered.map((m) => <button key={m.id} type="button" onClick={() => setSelectedId(m.id)} className="block w-full px-6 py-5 text-left hover:bg-slate-50"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">{m.committee.name}</p><Badge tone={m.status === "CANCELLED" ? "danger" : m.status === "CLOSED" ? "gold" : "success"}>{meetingStatusLabel(m.status)}</Badge></div><h3 className="mt-1 text-lg font-semibold">{m.title}</h3><p className="mt-1 text-sm text-slate-600">{formatDateTime(m.startAt, m.timezone)} - {m.location}</p></div><span className="text-sm font-semibold text-slate-500">{m.agendaItems.length} agenda {m.agendaItems.length === 1 ? "item" : "items"}</span></div></button>)}</div>}</section></div>; }
      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-center">
        ...
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
            ...
            Committee Register. Authentication and committee
...
function MeetingDetail({ meeting, user, onBack, onRefresh }: { meeting: Meeting; user: { id: string; role: UserRole }; onBack: () => void; onRefresh: () => Promise<void> }) { const [showEdit, setShowEdit] = useState(false); const [showCancel, setShowCancel] = useState(false); const [working, setWorking] = useState(false); const [errorMessage, setErrorMessage] = useState(""); const [message, setMessage] = useState(""); const canManage = user.role === "ADMIN"; async function cancelMeeting() { setWorking(true); setErrorMessage(""); try { const response = await apiRequest<ApiResponse<Meeting>>(`/api/meetings/${encodeURIComponent(meeting.id)}`, { method: "DELETE" }); await onRefresh(); setShowCancel(false); setMessage(response.warnings?.length ? `Meeting cancelled. ${response.warnings.join(" ")}` : "Meeting cancelled."); } catch (e) { setErrorMessage(e instanceof Error ? e.message : "Unable to cancel meeting."); } finally { setWorking(false); } } if (showEdit) return <MeetingForm committees={[meeting.committee]} initialMeeting={meeting} onCancel={() => setShowEdit(false)} onSaved={async () => { setShowEdit(false); await onRefresh(); }} />; return <div className="space-y-6"><Button variant="ghost" onClick={onBack}>Back to meetings</Button>{message && <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{message}</div>}{errorMessage && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{errorMessage}</div>}<section className="rounded-xl border border-slate-200 bg-white p-7 shadow-sm"><div className="flex flex-col justify-between gap-6 lg:flex-row"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold uppercase tracking-[0.10em] text-slate-500">{meeting.committee.name}</p><Badge tone={meeting.status === "CANCELLED" ? "danger" : meeting.status === "CLOSED" ? "gold" : "success"}>{meetingStatusLabel(meeting.status)}</Badge></div><h2 className="mt-2 text-[26px] font-semibold">{meeting.title}</h2>{meeting.description && <p className="mt-4 max-w-3xl whitespace-pre-wrap text-sm leading-6 text-slate-600">{meeting.description}</p>}</div>{canManage && meeting.status === "SCHEDULED" && <div className="flex gap-2"><Button onClick={() => setShowEdit(true)}>Edit</Button><Button variant="danger" onClick={() => setShowCancel(true)}>Cancel meeting</Button></div>}</div></section><section className="grid gap-4 md:grid-cols-2"><DetailCard label="Date" value={formatDate(meeting.startAt, meeting.timezone)} /><DetailCard label="Time" value={`${formatTime(meeting.startAt, meeting.timezone)} - ${formatTime(meeting.endAt, meeting.timezone)} (${meeting.timezone})`} /><DetailCard label="Location" value={meeting.location} /><DetailCard label="Committee" value={meeting.committee.name} /></section>{canManage && meeting.status === "SCHEDULED" && <MeetingCheckInQr meeting={meeting} />}<section className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-6 py-5"><h3 className="font-semibold">Agenda</h3><p className="mt-1 text-sm text-slate-500">Agenda for this meeting.</p></div><div className="divide-y divide-slate-100">{meeting.agendaItems.map((item) => <div key={item.id ?? item.position} className="flex gap-4 px-6 py-5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 text-sm font-bold text-slate-600">{item.position}</div><div><h4 className="font-semibold">{item.title}</h4>{item.description && <p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600">{item.description}</p>}</div></div>)}</div></section><AttendancePanel meeting={meeting} user={user} /><ConflictOfInterestPanel meetingId={meeting.id} timezone={meeting.timezone} user={user} /><DocumentsPanel committeeId={meeting.committeeId} meetingId={meeting.id} meetingStatus={meeting.status} user={user} /><section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h3 className="font-semibold">Meeting history</h3><div className="mt-4 grid gap-4 sm:grid-cols-2"><DetailCard label="Created" value={formatDateTime(meeting.createdAt, meeting.timezone)} /><DetailCard label="Last updated" value={formatDateTime(meeting.updatedAt, meeting.timezone)} />{meeting.closedAt && <DetailCard label="Closed" value={formatDateTime(meeting.closedAt, meeting.timezone)} />}{meeting.cancelledAt && <DetailCard label="Cancelled" value={formatDateTime(meeting.cancelledAt, meeting.timezone)} />}</div></section>{showCancel && <Modal title="Cancel this meeting?" onClose={() => !working && setShowCancel(false)}><p className="mt-3 text-sm leading-6 text-slate-600">This will remove the calendar event and mark the meeting as cancelled. Existing attendance and apologies will remain on record.</p><div className="mt-6 flex justify-end gap-2"><Button disabled={working} onClick={() => setShowCancel(false)}>Keep meeting</Button><Button variant="danger" disabled={working} onClick={() => void cancelMeeting()}>{working ? "Cancelling " : "Cancel meeting"}</Button></div></Modal>}</div>; }
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-7 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.10em] text-slate-500">
            {committeeTypeLabel(selected.type)}
          </p>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Committee information and shared documents.
          </p>
        </section>
    ...
    <div className="space-y-6">
      <section>
        ...
        <p className="mt-2 text-sm text-slate-600">
          Open a committee to view its information and shared documents.
        </p>
      </section>
      ...
      {committees.length === 0 ? (
        <EmptyState
          ...
          description="No committees are available to you yet."
        />
      ) : (
          ...
          {committees.map(
            (committee) => (
              <button
                ...
                className="rounded-xl border border-slate-200 bg-white p-6 text-left shadow-sm transition hover:border-slate-300 hover:bg-slate-50"
              >
                <p className="text-xs font-semibold uppercase tracking-[0.10em] text-slate-500">
                  {committeeTypeLabel(committee.type)}
                </p>
